...
DATA_CACHE_CONFIG = {
    ...
    # chart data payloads are not JSON serializable, so opt in to the pickle codec
    "CODEC": PickleKeyValueCodec(),
}
```

The built-in cache serializes values with the JSON codec (`JsonKeyValueCodec`) unless a `CODEC` is
provided. The pickle codec (`PickleKeyValueCodec`) can store arbitrary Python objects, but
deserializing pickled data from the metadata database is only as safe as that database, so it must
