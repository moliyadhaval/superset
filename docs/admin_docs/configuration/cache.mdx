...
The built-in cache serializes entries with `JsonKeyValueCodec` unless `CODEC` says
otherwise. Chart data payloads are not JSON-serializable, so this config selects
`PickleKeyValueCodec` explicitly. Only use it when the metadata database is fully
within the operator's trust boundary, since decoding a pickled value executes the
data it reads.
