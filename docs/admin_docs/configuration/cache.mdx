...
DATA_CACHE_CONFIG = {
    ...
    "CODEC": PickleKeyValueCodec(),  # chart data is not JSON-serializable
}
```

The built-in cache serializes values with `JsonKeyValueCodec` unless a `CODEC` is
provided. Caches holding non-JSON-serializable values (such as chart data) must set
`"CODEC": PickleKeyValueCodec()` (`from superset.key_value.types import
PickleKeyValueCodec`). Only do so when the metadata database is fully trusted, as
unpickling data is unsafe if the stored bytes can be tampered with.
